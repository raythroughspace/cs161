...
uintptr_t proc::syscall(regstate* regs) {
    ...
    case SYSCALL_MAP_CONSOLE:{
        uintptr_t addr = regs->reg_rdi;
        if (addr % PAGESIZE != 0 || addr > VA_LOWMAX){
            return -1;
        }
        void* pg = kalloc(PAGESIZE);
        int r = vmiter(this, addr).try_map(ka2pa(pg), PTE_U|PTE_W|PTE_P);
        if (r){ //failed mapping
            return -1;
        }
        return 0;
    ...
    case SYSCALL_NASTY:{
        ...
        assert(sizeof(struct proc) == 4096);
        for (size_t i = 0; i< sizeof(struct proc); i++){
            buf[i] = i;
        }
        return buf[0];
    }
...
int proc::syscall_fork(regstate* regs) {
    ...
    for (pid_t i=1; i<NPROC; ++i){
        if (!ptable[i]){ //entry i is not used
            cpid = i;
            break;
        }
    }
    ...
    for (vmiter it(current()->pagetable_, 0); it.va() < MEMSIZE_VIRTUAL; it.next()){
            if (it.user() && it.writable() && it.va() != CONSOLE_ADDR){
                void* pa = kalloc(PAGESIZE);
                assert(pa);
                memcpy(pa, (void*) it.va(), PAGESIZE);
                log_printf("User writable mapping at va: %p, pa: %p, perm: %d\n", it.va(), ka2pa(pa), it.perm());
                int r = vmiter(cpt, it.va()).try_map(ka2pa(pa), it.perm());
                assert(!r);

            }
            else{
                log_printf("User non writable mapping at va: %p, pa: %p, perm: %d\n", it.va(), it.pa(), it.perm());
                int r = vmiter(cpt, it.va()).try_map(it.pa(), it.perm());
                assert(!r);
            }
    }
    proc* p = knew<proc>();
    p->init_user(cpid, cpt);
    ptable[cpid] = p;
    memcpy((void*) p->regs_, (void*)regs, sizeof(regstate));
    p->regs_->reg_rax = 0;
    cpus[cpid % ncpu].enqueue(p);
    return cpid;
}
